...
findSleepiestGuard : List GuardSleepData -> GuardSleepData
findSleepiestGuard guards =
    List.foldl 
                (\value max ->
                    if value.totalMinutesAsleep > max.totalMinutesAsleep then
                        value
                    else
                        max
                ) defaultGuardSleepData guards

calculateAnswer : GuardSleepData -> Int
calculateAnswer guard =
    let 
        sleepiestMinute = 
            Dict.foldl
                (\key value max ->
                    if value > Tuple.second max then 
                        (key, value)
                    else 
                        max
                ) (0,0) guard.sleepByMinute

        log = Debug.log "sleepiestMinute" sleepiestMinute
    in
        guard.id * Tuple.first sleepiestMinute
