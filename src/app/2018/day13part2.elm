...
calculate inputVal =
    ...
    in
        ...
        |> (\result ->
                (Tuple.second result.finalPosition, Tuple.first result.finalPosition)
            )

letTheCartsRoll : { a | tracks : Array String, carts : List Cart } -> { tracks : Array String, carts : List Cart, finalPosition : (Int, Int) }
letTheCartsRoll currentState =
    let
        ...
        updatedState = 
            List.foldl
                (\cart result ->
                    if List.member cart.position result.deadCarts then
                        result
                    else
                        let
                            ...
                            deadCarts = 
                                if resultantCollision /= (-1, -1) then
                                    result.deadCarts ++ [resultantCollision]
                                else
                                    result.deadCarts
                            ...
                            updatedCarts =
                                if resultantCollision == (-1, -1) then
                                    ...
                                    List.filter 
                                        (\deadCart ->
                                            deadCart.position /= resultantCollision
                                        ) remainingCarts 

                            isFinalIteration = 
                                List.length updatedCarts == 1
                ...
                ) { deadCarts = [], carts = sortedCarts, isFinalIteration = False} sortedCarts

        finalPosition =
            if updatedState.isFinalIteration then
                case List.head updatedState.carts of
                    Just value ->
                        value.position
                    Nothing ->
                        (-1,-1)
